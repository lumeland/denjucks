...
var Parser = Object.extend({
  ...
  parseCall: function () {
    ...
    var callerNode = new nodes.Caller(
      callTok.lineno,
      callTok.colno,
      callerName,
      callerArgs,
  ...
  parseFrom: function () {
    ...
    while (1) {
      ...
      if (name.value.charAt(0) === "_") {
        this.fail(
          "parseFrom: names starting with an underscore " +
            "cannot be imported",
          name.lineno,
          name.colno,
        );
  ...
  parseIf: function () {
    ...
    if (
      this.skipSymbol("if") || this.skipSymbol("elif") ||
    ...
    switch (tok && tok.value) {
      ...
      default:
        this.fail(
          "parseIf: expected elif, else, or endif, " +
            "got end of file",
        );
    }
  ...
  parseRaw: function (tagName) {
    ...
    while (
      (matches = this.tokens._extractRegex(rawBlockRegex)) && rawLevel > 0
    ) {
      var all = matches[0];
      var pre = matches[1];
