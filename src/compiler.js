...
import _Frame_ from "./runtime.js";
let Frame = _Frame_.Frame;
...
var Compiler = Object.extend({
  ...
  compileCallExtension: function (node, frame, async) {
    ...
    if (async) {
      ...
      this.emitLine(
        this.buffer + " += runtime.suppressValue(" + res + ", " + autoescape +
          " && env.opts.autoescape);",
      );
      this.addScopeLevel();
    } else {
    ...
    if (node.name instanceof nodes.Array) {
      ...
      this.emitLine("if(runtime.isArray(" + arr + ")) {");
      {
        ...
        for (var u = 0; u < node.name.children.length; u++) {
          ...
          this.emitLine(
            'frame.set("' + node.name.children[u].value +
              '", ' + arr + "[" + i + "][" + u + "]" + ");",
          );
  ...
  _compileAsyncLoop: function (node, frame, parallel) {
    ...
    if (node.name instanceof nodes.Array) {
      ...
      this.emitLine(
        "runtime." + asyncMethod + "(" + arr + ", 1, function(" + id + ", " +
          i + ", " + len + ",next) {",
      );
      this.emitLine('frame.set("' + id + '", ' + id + ");");
      frame.set(id, id);
  ...
  compileImport: function (node, frame) {
    ...
    this.emitLine(
      ", false, " + this._templateName() + ", false, " + this.makeCallback(id),
    );
    this.addScopeLevel();
  ...
  compileFromImport: function (node, frame) {
    ...
    this._compileExpression(node.template, frame);
    this.emitLine(
      ", false, " + this._templateName() + ", false, " +
        this.makeCallback(importedId),
    );
    this.addScopeLevel();

    this.emitLine(
      importedId + ".getExported(" +
        (node.withContext ? "context.getVariables(), frame, " : "") +
        this.makeCallback(importedId),
    );
    this.addScopeLevel();
  ...
  compileExtends: function (node, frame) {
    ...
    this._compileExpression(node.template, frame);
    this.emitLine(
      ", true, " + this._templateName() + ", false, " +
        this.makeCallback("_parentTemplate"),
    );
  ...
  compileInclude: function (node, frame) {
    ...
    this._compileExpression(node.template, frame);
    this.emitLine(
      ", false, " + this._templateName() + ", " + node.ignoreMissing + ", " +
        this.makeCallback(id),
    );
...
export default {
  compile: function (src, asyncFilters, extensions, name, opts) {
    ...
    c.compile(transformer.transform(
      parser.parse(src, extensions, opts),
      asyncFilters,
      name,
    ));
    return c.getCode();
